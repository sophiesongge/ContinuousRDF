...
public class BoltBuilderWithThreeBF implements IRichBolt {
	...
	public void execute(Tuple input) {
		 ...
		 * 
		 * if you run oneVariableJoin Query Result will be : [Sophie, Justine,
		 * Fabrice] if you run twoVariableJoin Query Result will be : [Sophie,
		 * Justine, Fabrice, Lea] if you run multiVariableJoin Query Result will
		 * be : [Sophie, Justine, Fabrice, Lea, Frederic]
		 * 
		 */
	...
	public void query(String Subject, String Predicate, String Object) {
		...
		// ("1-variable join, to find the authors for paper kNN who works in
		// INRIA and who has a Ph.D diplome:");
		// find s where p1=v1,p2=v2,p3=v3;

		// ("2-variable join, to find the authors for paper kNN who works in
		// INRIA and their diplome:");
		// find s where p1=v1,p2=v2,p3=any;

		// ("multi-variable join, to find the authors for paper kNN, and the
		// place they work, and their diplome: ");
		...
		String objectPaper = "kNN", objectWork = "INRIA", objectDiplome = "ANY";// start
																				// with
																				// all
																				// values
																				// ANY
		...
		for (int i = 0; i < v.length; i++) {
			...
			// note: if it doesn't fit with a value, the object just stays at
			// any
		}
		...
		// Identify the join type and set values of predicate and objects
		// accordingly
		int countAny = 0;
		int index = 0;
		...
		/*
		 * for(int i=0;i<3;i++) { System.out.println(predicates[i]);
