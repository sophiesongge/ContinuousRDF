...
public class API {
	...
	public static void readFile() {
		/*
		 * Steps todo: 1: read file 2: convert the file into interpretable
		 * tuples and data files (source file: RDFTopology.java:26) 3: set up up
		 * the topology to read the interpretable data files 4: run the topology
		 * for this 5: for each tuple: get the results and write them to a file
		 */
		/*
		 * System.out.println("readFile called"); // create an empty model Model
		 * model = ModelFactory.createDefaultModel(); String
		 * inputFileName="./data/University0_0.daml"; // use the FileManager to
		 * find the input file InputStream in = FileManager.get().open(
		 * inputFileName ); if (in == null) { throw new
		 * IllegalArgumentException( "File: " + inputFileName + " not found"); }
		 * // read the RDF/XML file model.read(in, null);
		 * 
		 * 
		 * // list the statements in the Model StmtIterator iter =
		 * model.listStatements(); // print out the predicate, subject and
		 * object of each statement while (iter.hasNext()) { Statement stmt =
		 * iter.nextStatement(); // get next statement Resource subject =
		 ...
		 * 
		 * try { return runQuery(input); } catch (Exception e) { // TODO
