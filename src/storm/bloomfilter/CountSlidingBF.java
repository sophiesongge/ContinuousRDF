...
public class CountSlidingBF<E> {
	...
	public int generationSize; // the maximum number of elements a generation
								// will contain (The number of elements need to
								// be update each time)
	public int numberOfGeneration; // the number of generation in a Sliding
									// Window
	public int slidingWindowSize; // the maximum number of elements a sliding
									// window will contain
	public int elementNumber; // the number of elements we have already treated
								// in a Sliding Window
	public int k; // the number of hash functions
	public BloomFilter<E> BloomFilter; // the Bloom Filter used in this Sliding
										// Window
	public Map<Integer, Integer> Generation; // the Map to store the information
												// of generation, with key:hash
												// value (bitset index);
												// value:generation ----
	...
	/*
	 * public void add(String str){ int[] hashes =
	 * this.BloomFilter.createHashes(str.getBytes(charset),
	 * this.BloomFilter.getK()); int generationNumber =
	 * (int)Math.floor(elementNumber / generationSize); for(int hash : hashes){
	 * this.Generation.put(Math.abs(hash % this.BloomFilter.bitSetSize),
	 * generationNumber); } ArrayList<Integer> index = new ArrayList<Integer>();
	 * for(Integer key : Generation.keySet()){ if(Generation.get(key)==0){
	 * index.add(key); } } BloomFilter.add(str); elementNumber ++;
	 * if(elementNumber > slidingWindowSize){ for(int i=0; i<index.size(); i++){
	 * BloomFilter.setBit(index.get(i), false); } elementNumber = 1; } }
	 */
